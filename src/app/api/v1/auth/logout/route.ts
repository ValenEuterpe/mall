 ...
 */
async function parseLogoutOptions(
  request: NextRequest
): Promise<LogoutOptions> {
  try {
    const body = await request.json();
    return {
      allDevices:
        typeof body.allDevices === "boolean" ? body.allDevices : false,
    };
  } catch {
...
async function safeClearCookies(): Promise<void> {
  try {
    ...
    console.error(
      "Failed to clear cookies during error handling:",
      cookieError
    );
  }
}
