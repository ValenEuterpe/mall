...
function extractMultilingualFields(data: Record<string, unknown>) {
    ...
    if (data.description && !description_en && !description_ru && !description_am) {
    ...
    if (data.detailDescription && !detailDescription_en && !detailDescription_ru && !detailDescription_am) {
    ...
    return {
        ...
        detailDescription_en, detailDescription_ru, detailDescription_am,
    };
}
...
export async function createProductHandler(request: NextRequest): Promise<NextResponse> {
    ...
    if (data.subcategoryId) {
        ...
        if (data.categoryId && subcategory.categoryId !== data.categoryId) {
            throw new ForbiddenError("Subcategory does not belong to selected category");
        }
    }
    ...
    if (data.subSubcategoryId) {
        ...
        if (data.subcategoryId && subSubcategory.subcategoryId !== data.subcategoryId) {
            throw new ForbiddenError("Sub-subcategory does not belong to selected subcategory");
        }
    }

    // Extract and prepare multilingual fields
    let multilingualFields = extractMultilingualFields(data as Record<string, unknown>);
    ...
    if (data.autoTranslate && isTranslationAvailable()) {
        ...
        const sourceName = multilingualFields.name_en || multilingualFields.name_ru || multilingualFields.name_am;
        const sourceDesc = multilingualFields.description_en || multilingualFields.description_ru || multilingualFields.description_am;
        const sourceDetail = multilingualFields.detailDescription_en || multilingualFields.detailDescription_ru || multilingualFields.detailDescription_am;
        ...
        if (sourceName) {
            try {
                ...
                if (translations.name) {
                    multilingualFields.name_en = translations.name.en || multilingualFields.name_en;
                    multilingualFields.name_ru = translations.name.ru || multilingualFields.name_ru;
                    multilingualFields.name_am = translations.name.am || multilingualFields.name_am;
                }
                if (translations.description) {
                    multilingualFields.description_en = translations.description.en || multilingualFields.description_en;
                    multilingualFields.description_ru = translations.description.ru || multilingualFields.description_ru;
                    multilingualFields.description_am = translations.description.am || multilingualFields.description_am;
                }
                if (translations.detailDescription) {
                    multilingualFields.detailDescription_en = translations.detailDescription.en || multilingualFields.detailDescription_en;
                    multilingualFields.detailDescription_ru = translations.detailDescription.ru || multilingualFields.detailDescription_ru;
                    multilingualFields.detailDescription_am = translations.detailDescription.am || multilingualFields.detailDescription_am;
                }
                ...
                logger.warn("Auto-translation failed, proceeding without translation", {
                    sellerId: user.userId,
                    error,
                });
            }
        ...
        const product = await prisma.$transaction(async (tx) => {
            ...
            const newProduct = await tx.product.create({
                data: {
                    ...
                    ...multilingualFields,
                    // Legacy name field for backward compatibility (use first available)
                    name: multilingualFields.name_en || multilingualFields.name_ru || multilingualFields.name_am || "",
                    description: multilingualFields.description_en || multilingualFields.description_ru || multilingualFields.description_am,
                    detailDescription: multilingualFields.detailDescription_en || multilingualFields.detailDescription_ru || multilingualFields.detailDescription_am,
                    shopId: shop.id,
                    status: data.status || ProductStatus.DRAFT,
                },
                include: {
                    category: {
                        select: { id: true, key: true, name_en: true, name_ru: true, name_am: true },
                    },
                    subcategory: {
                        select: { id: true, key: true, name_en: true, name_ru: true, name_am: true },
                    },
