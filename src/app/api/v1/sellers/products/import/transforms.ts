...
export function transformRowToProduct(
    ...
    const categoryName = row.category ? String(row.category).trim().toLowerCase() : null;
    const categoryKey = categoryName && categoryMap.has(categoryName)
        ? categoryMap.get(categoryName)!
        : null;

    // Parse status
    const rawStatus = row.status ? String(row.status).trim().toUpperCase() : "DRAFT";
