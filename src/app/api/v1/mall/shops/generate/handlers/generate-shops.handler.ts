...
export async function generateShopsHandler(request: NextRequest): Promise<NextResponse> {
    const user = requireAuth(request, ["MALL_OWNER"]);
    const data = await validateBody(request, shopGenerateSchema);

    const { venue, building, floor, floorId, shopTypeId, startNumber, count, numberDigits = 2 } = data;
    ...
    for (let i = 0; i < count; i++) {
        ...
        const fullCode = generateShopCode(venue, building ?? null, floor ?? null, shopNumber);
        const svgId = generateSvgId(venue, building ?? null, floor ?? null, shopNumber);
