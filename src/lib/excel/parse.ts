...
export function parseExcelFile<T extends ExcelRow = ExcelRow>(
    ...
    const rawData = XLSX.utils.sheet_to_json<Record<string, unknown>>(sheet, sheetToJsonOpts);
    ...
    const normalizedHeaders = normalizeHeaders(originalHeaders, opts.columnMapping);
...
export function parseExcelFileWithErrors<T extends ExcelRow = ExcelRow>(
    buffer: Buffer,
    options: ParseOptions = {},
    rowValidator?: (row: ExcelRow, index: number) => { valid: boolean; errors: string[] }
): {
    data: T[];
