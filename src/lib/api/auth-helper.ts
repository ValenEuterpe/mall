...
import type { UserRole, AuthenticatedUser, AuthErrorCode, RequestContext } from "@/types/auth";
...
export function extractAccessToken(request: NextRequest): string | null {
    ...
    const cookieToken = request.cookies.get(cookieConfig.names.accessToken)?.value;
    if (cookieToken) {
        return cookieToken;
    }
